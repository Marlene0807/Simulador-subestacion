...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const simular2 = document.getElementById('Simulador2');
...
function prototipoGuardadoArchivo(){
    ...
    images.forEach(image => {
        ...
        if(image.tipe=="Tierra"){          
            const textConcatenado = (image.tipe+"|"+image.x+"|"+image.y+"|"+image.width+"|"+image.idComponent+ " ");
            textos.push(textConcatenado);  
        }
...
guardarArrayEnTxt(textos, 'Archivo-Guardado.txt');
}
...
//Selector de un objeto al ser seleccionado mediante el despliegue de la lista
    ...
    if(selectedImage.tipe=="Interruptor"){
        if(EA=="off"){
            selectedImage.estatus="on";
    ...
    function iniciarS(){
        simOF=1;
        simular2.removeAttribute('hidden');
        simular.setAttribute('hidden',true);
        images.forEach(image => {
            if(image.tipe=="Alimentador-Comun"){
                image.estatus="on";
    ...
    function apagarS(){  
        simOF=0;    
        simular.removeAttribute('hidden');
        simular2.setAttribute('hidden',true);
        images.forEach(image => {
            if(image.tipe=="Barra"){
                image.estatus="off";
...
/*
    ...
    simular2.addEventListener('click', apagarS);
